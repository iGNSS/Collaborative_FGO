 ...
 * 
 * Main fucntions: pseudorange/carrier/Doppler velocity fusion using factor graph optimization 
 * input: pseudorange (code), carrier-phase (phase) and Doppler velocity from GPS/BeiDou.
 * output: position of the GNSS receiver 
 * Notes: Typically, the carrier-phase is used in GNSS-RTK and the integer ambiguity is estimated (maybe only float ambigurity). To use the carrier-phase in non-RTK, we proposes to make use of the unary matrix to enable the ambigurity-free usage of carrier-phase measurements.
 * Ref: MSCKF for visual/inertial integrated positioning
 * Date: 2021/01/17
 *******************************************************/
...
#include "../../include/gnss_estimator/psr_car_doppler_fusion.h"

class psr_car_doppler_fusion
{
    ros::NodeHandle nh;
    ...
    /* path to save the WCP size */
    std::string logWCPSizePath;
...
public:
    psr_car_doppler_fusion()
    {
        /* setup logpath */
        ros::param::get("logPath", logPath);
        ros::param::get("logWCPSizePath", logWCPSizePath);

        /* thread for factor graph optimization */
        optimizationThread = std::thread(&psr_car_doppler_fusion::solveOptimization, this);
        ...
        syncdoppler2GNSSRaw->registerCallback(boost::bind(&psr_car_doppler_fusion::gnssraw_doppler_msg_callback,this, _1, _2));
    ...
    void solveOptimization()
    {
        ...
        /* clear the data stream in continuous carrier-phase chain*/
        factor_graph.continuousCarChain.clearDataStream();
        ...
        while(1)
        {
            ...
            if(factor_graph.getDataStreamSize()>3 && hasNewData)
            {
                ...
                /* add ambiguity free carrier-phase FACTORS to factor graph */
                factor_graph.addCarrierPhaseFactors(problem);
                ...
                /* log result */
                factor_graph.logWCPSize(logWCPSizePath);
    ...
    void gnssraw_doppler_msg_callback(const nlosExclusion::GNSS_Raw_ArrayConstPtr& gnss_msg, const nav_msgs::OdometryConstPtr& doppler_msg)
    {
        ...
        if(checkValidEpoch(time_frame) && m_GNSS_Tools.checkRepeating(*gnss_msg))
        {
            if(gnss_msg->GNSS_Raws.size())
            {
                ...
                /* input data to continuous carrier-phase chain */
                factor_graph.continuousCarChain.input_gnss_raw_data(*gnss_msg, time_frame);
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "psr_car_doppler_fusion_node"); 
    ROS_INFO("\033[1;32m----> psr_car_doppler_fusion_node Started.\033[0m"); 
    // ...
    psr_car_doppler_fusion psr_car_doppler_fusion_;
    ros::spin();
    return 0;
}
