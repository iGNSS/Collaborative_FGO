    ...
    void getGroundTruthSolutionfromCSV()
    {
        // while(1)
        {
            ...
            if(!m_gt_solutions_Vec.size())
            {
                ...
                Solution_.gps_week =strtod((result[1]).c_str(), NULL);
                Solution_.gps_sec =int(strtod((result[0]).c_str(), NULL));

                double lat_D = strtod((result[2]).c_str(), NULL);
                double lat_M = strtod((result[3]).c_str(), NULL);
                double lat_S = strtod((result[4]).c_str(), NULL);
                Solution_.latitude = lat_D + lat_M/60 + lat_S/3600;

                double lon_D = strtod((result[5]).c_str(), NULL);
                double lon_M = strtod((result[6]).c_str(), NULL);
                double lon_S = strtod((result[7]).c_str(), NULL);
                Solution_.longitude = lon_D + lon_M/60 + lon_S/3600;

                Solution_.altitude = strtod((result[8]).c_str(), NULL);
                std::cout << std::setprecision(17);
