...
double removeLowSnrSat(nlosExclusion::GNSS_Raw_Array& GNSS_data) //
{
  bool result = true;
  ...
  for(int i =0; i < GNSS_data.GNSS_Raws.size(); i++) // 
  {
    ...
    if(GNSS_data.GNSS_Raws[i].snr<miniSnr)
    {
      miniSnr = GNSS_data.GNSS_Raws[i].snr;
      index = i;
    }
  ...
  if(index>=0)
  {
    GNSS_data.GNSS_Raws.erase(GNSS_data.GNSS_Raws.begin()+index);
  }
...
/**
  ...
  */
  Eigen::MatrixXd WeightedLeastSquare(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, nlosExclusion::GNSS_Raw_Array GNSS_data, std::string method){
    ...
    for (int idx = 0; idx < eWLSSolution.rows(); idx++){
      eWLSSolution(idx, 0) = 0;
      // eWLSSolution(idx, 0) = 21458544.0506 * 100000; // non-convex guess
    }
    ...
    // Eigen::MatrixXd hessian = (eH_Matrix_.transpose() * eH_Matrix_);
    // Eigen::EigenSolver<Eigen::Matrix<double, 5, 5>> esolver(hessian);
   ...
   * @brief  get the dop
   @ 
  */
  double getDop(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, nlosExclusion::GNSS_Raw_Array GNSS_data, std::string method){
    ...
    double dop = 0;
    Eigen::MatrixXd G_Matrix = (eH_Matrix_.transpose() * eH_Matrix_).inverse();
    std::cout << std::setprecision(12);

    dop = G_Matrix(0,0) + G_Matrix(1,1) + G_Matrix(2,2);
    dop = sqrt(dop);
