...
#include "../gnss_estimator/pseudorange_factor.h"
#include "../gnss_estimator/doppler_factor.hpp"
#include "../robust_models/gmm_adp.h"
#include "pseudorange_factor_gmm.h"
...
public:
    ...
    /* flag for the GMM availability */
    bool GMMAvailability = false;
...
public:
    ...
    bool clearVariables()
    {
        ...
        // for(int i = 0; i < psrIDs.size(); i++)
        // {
            
        //     problem.RemoveResidualBlock(psrIDs[i]);
        // }
        psrIDs.clear();
        satNumPerT.clear();
        
        state_gps_sec_vec.clear();
        GMMAvailability = false;
        return true;
    }
    ...
    bool setupSolverOptions(ceres::Solver::Options& options)
    {
        ...
        /* maximum iterations of 15 is enough (accuracy is better than 258 iterations) */
        ...
        for(int m = 0;  m < length; m++,iter_pr++) // 
        {
            ...
            for(int i = 0; i < Ps.size(); i++)
            {
                if(time == Ps[i].first)
                {
                    ...
                    state_array[m][0] = eWLSSolutionECEF(0);
                    state_array[m][1] = eWLSSolutionECEF(1);
                    state_array[m][2] = eWLSSolutionECEF(2);
                    state_array[m][3] = eWLSSolutionECEF(3);
                    state_array[m][4] = eWLSSolutionECEF(4);
                }
            } 
        }
        return true;
    ...
    bool addPseudorangeFactors(ceres::Problem& problem, std::string errorModel)
    {
        ...
        for(int m = 0;  m < length; m++,iter_pr++) // 
        {
            ...
            for(int i =0; i < sv_cnt; i++)
            {
                ...
                else if(errorModel == "GMM")
                {
                    libRSF::GaussianMixture<1> GMMTmp;
                    libRSF::GaussianMixture<1> mThGMM = gaussianMixtureModel.GMMSets[m].second.GMMPara;
                    if(mThGMM.getNumberOfComponents()>=1)
                    {
                        GMMTmp = mThGMM;
                    }
                    else
                    {
                        std::cout <<" number of Gaussian Component is less than 1->  " << m << "  " <<mThGMM.getNumberOfComponents()<< std::endl;
                    ...
                    auto ID = problem.AddResidualBlock(ps_function_GMM, loss_function, state_array[m]);
                    psrIDs.push_back(ID);
            ...
            satNumPerT.push_back(std::make_pair(t, sv_cnt));
        }
    ...
    /* update satNumPerT */
    bool updatesatNumPerT()
    {
        /* estimate the GMM with Expectation Maximization */
        gaussianMixtureModel.updatesatNumPerT(satNumPerT);
    ...
    /* set the GMM availability */
    bool setGMMAvailability()
    {
        if(measSize > gaussianMixtureModel.GMMResidualWindowSize)
        {
            GMMAvailability = true;
        }
        return true;
    }
    ...
    bool updateResidualsOfGMM(ceres::Problem& problem)
    {
        ...
        gaussianMixtureModel.updateGMMResiduals(residuals);
        return true;
    }
    ...
    /* update GMM sets with EM */
    bool estimateGMMSetsViaEM()
    {
        /* estimate the GMM sets with Expectation Maximization */
        gaussianMixtureModel.estimateGMMSetsParas();

        return true;
    }

    /* removePseudorangeFactors */
    bool removePseudorangeFactors(ceres::Problem& problem)
    {
        for (int i = 0; i < psrIDs.size(); i++)
        {
            problem.RemoveResidualBlock(psrIDs[i]);
