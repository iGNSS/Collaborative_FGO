...
#include "continuous_car_chain.h"
...
public:
    ...
    /* continuous carrier-phase chain for manage carrier-phase measurements*/
    ContinuousCarChain continuousCarChain;
...
public:
    ...
    /* add ambiguity free carrier-phase FACTORS */
    bool addCarrierPhaseFactors(ceres::Problem& problem)
    {
        /* get satellite Prns */
        continuousCarChain.getSatPrns();

        /* construct continuous carrier-phase chains (CCC) */
        continuousCarChain.constructCCC();

        /* add the CCC factors */
        int length = continuousCarChain.CCCs.size();
        if (length<3) return false;
        for(int i = 0; i < length; i++) 
        {
            singleContinuousCarChain SCCC = continuousCarChain.CCCs[i];
            std::vector<double*> parameter_blocks;
            ceres::LossFunction *loss_function_Carrier;
            loss_function_Carrier = new ceres::CauchyLoss(3.0);
   ...
   */
    ...
    /* LOG results */
    void logWCPSize(std::string logWCPSizePath)
    {
        std::ofstream foutC(logWCPSizePath, std::ios::ate);
        foutC.setf(std::ios::fixed, std::ios::floatfield);
        ...
        for(int m = 0;  m < length; m++) // 
        {
            ...
            /* gps time */
            foutC.precision(0);             
            foutC<<continuousCarChain.CCCs[m].length<<",";
            foutC<<std::endl;
        }
        std::cout << "logWCPSizePath-> "<< logWCPSizePath<< std::endl;
        foutC.close();
    }
