...
#define Doppler2PSRWeight 100 // 50 100
        ...
        for(int m = 0;  m < length; m++,iter_pr++) // 
        {
            ...
            // if(iter_prNext != gnss_raw_map.end())
            {
                ...
                for(int i =0; i < sv_cnt; i++)
                {
                    ...
                    // doppler = gnss_data.GNSS_Raws[i].doppler;
    ...
    bool addMotionModelFactors(ceres::Problem& problem)
    {
        ...
        for(iter_pr = gnss_raw_map.begin(); iter_pr != gnss_raw_map.end();iter_pr++, i++)
        {
            ...
            if(iter_prNext != gnss_raw_map.end())
            {
                ...
                ceres::CostFunction* motionModel_function = new ceres::AutoDiffCostFunction<motionModelFactor, 3 
                                                        , 6,6,3>(new 
                                                        motionModelFactor(delta_t, 0.6 * 0.5));
                problem.AddResidualBlock(motionModel_function, loss_function, state_array[i],state_array[i+1],state_array_vel[i]);
            }
        }
        return true;
    }
